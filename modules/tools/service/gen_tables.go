...
import (
	...
	"github.com/mooncake9527/orange/modules/tools/models/tools"
	"github.com/mooncake9527/orange/modules/tools/service/dto"
	...
	"github.com/mooncake9527/orange-core/common/utils/files"
...
func (e *GenTablesService) GenTableInit(dbname string, tableName string, force bool) (models.GenTables, error) {
	...
	if dbname == "master" {
		data.PackageName = consts.DbDefault
	} else {
	...
	for i := 0; i < len(dbcolumn); i++ {
		...
		if column.ColumnName == "updated_at" {
			...
			column.IsList = "1"
		} else if strings.Contains(dbcolumn[i].ColumnKey, "tinyint unsigned") {
			column.GoType = "uint8"
...
func (e *GenTablesService) NOMethodsGen(tab models.GenTables, force bool) error {
	...
	type RendFile struct {
		Temp string
		Out  string
	}

	var iRendFiles = []RendFile{
		// go
		{
			Temp: "go/router/cmd_api.template",
			Out:  "cmd/start/" + tab.PackageName + ".go",
		},
		{
			Temp: "go/router/router.template",
			Out:  ROOT + tab.PackageName + "/router/router.go",
		},
		{
			Temp: "go/service/model.go.template",
			Out:  ROOT + tab.PackageName + "/models/" + tab.TBName + ".go",
		},
		{
			Temp: "go/service/apis.go.template",
			Out:  ROOT + tab.PackageName + "/apis/" + tab.TBName + ".go",
		},
		{
			Temp: "go/service/router_no_check_role.go.template",
			Out:  ROOT + tab.PackageName + "/router/" + tab.TBName + ".go",
		},
		{
			Temp: "go/service/dto.go.template",
			Out:  ROOT + tab.PackageName + "/service/dto/" + tab.TBName + ".go",
		},
		{
			Temp: "go/service/service.go.template",
			Out:  ROOT + tab.PackageName + "/service/" + tab.TBName + ".go",
		},
		// front end
		{
			Temp: "vue/api/api.ts.template",
			Out:  frontPath + "/api/" + tab.PackageName + "/" + tab.MLTBName + ".ts",
		},
		{
			Temp: "vue/views/index.vue.template",
			Out:  frontPath + "/views/" + tab.PackageName + "/" + tab.MLTBName + "/index.vue",
		},
		{
			Temp: "vue/views/form.vue.template",
			Out:  frontPath + "/views/" + tab.PackageName + "/" + tab.MLTBName + "/form.vue",
		}, {
			Temp: "vue/views/utils/hook.tsx.template",
			Out:  frontPath + "/views/" + tab.PackageName + "/" + tab.MLTBName + "/utils/hook.tsx",
		},
	}

	for i, v := range iRendFiles {
		if i == 0 || i == 1 {
			err = render(basePath, v.Temp, v.Out, m)
		} else {
			err = render(basePath, v.Temp, v.Out, tab)
		}
		if err != nil {
			return err
		}
...
func render(basePath, tmp, out string, data any) error {
	t5, err := template.ParseFiles(basePath + tmp)
	if err != nil {
		return xerror.Wrap(err, "parse template")
	}
	var b5 bytes.Buffer
	err = t5.Execute(&b5, data)
	if err != nil {
		return xerror.Wrap(err, "execute template")
	}
	files.FileCreate(b5, out)
	slog.Info("render", "out", out)
	return nil
}
...
func GetDb(dbname string) (db *gorm.DB, mdb string, sdb, driver string) {
	...
	if dbname != consts.DbDefault {
		...
		xlog.Debug("driver", "test", gdsn.Driver)
